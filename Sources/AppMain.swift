...
struct HabitTrackerApp: App {
    ...
    var body: some Scene {
        Window("Session Manager", id: "manager") {
            SessionManagerWindowView()
        ...
        Window("Habit Dashboard", id: "dashboard") {
            RootView()
                .environmentObject(state)
...
enum AppTheme {
    static let accent = Color(red: 0.19, green: 0.68, blue: 0.90)
    static let secondary = Color(red: 0.36, green: 0.84, blue: 0.67)
    static let backgroundGradient = LinearGradient(
        colors: [Color(red: 0.90, green: 0.98, blue: 0.96), Color(red: 0.90, green: 0.96, blue: 1.0)],
...
final class AppState: ObservableObject {
    ...
    var menuBarTitle: String {
        if sessionState.mode == .idle { return "⏱ Idle" }
    ...
    func removeHabits(at offsets: IndexSet) {
        ...
        habitLogs.removeAll { log in !habits.contains(where: { $0.id == log.habitID }) }
        persist()
    }
    ...
    func logHabit(habit: Habit, date: Date, boolValue: Bool?, numberValue: Double?) {
        ...
        habitLogs.append(HabitLog(id: UUID(), habitID: habit.id, date: normalized, boolValue: boolValue, numberValue: numberValue))
        persist()
    }

    func habitLog(habitID: UUID, date: Date) -> HabitLog? {
        habitLogs.first { $0.habitID == habitID && Calendar.current.isDate($0.date, inSameDayAs: date) }
    }
...
struct SessionManagerWindowView: View {
    @EnvironmentObject private var state: AppState
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Quick navigation")
                .font(.headline)
            Button("Open full habit dashboard") {
...
struct QuickSessionPopupView: View {
    @EnvironmentObject private var state: AppState
    @Environment(\.openWindow) private var openWindow
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack {
                Button(state.sessionState.mode == .idle ? "Start" : (state.sessionState.isPaused ? "Resume" : "Pause")) {
                    if state.sessionState.mode == .idle {
                        state.startTimer()
                    } else {
                        state.pauseTimer()
                    }
                ...
                Button("Complete") {
                    let parsed = labels
                        ...
                        .filter { !$0.isEmpty }
                    state.completeCurrentSession(name: name, labels: parsed)
                }
                .disabled(state.sessionState.mode == .idle)
            }
            ...
            GroupBox("Session Metadata") {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Button("Complete current session") {
                        ...
                        let labels: [String]
                        if let chosenChore = selectedChore, !chosenChore.labels.isEmpty {
                            labels = chosenChore.labels
                        } else {
                            labels = customLabels
                                .split(separator: ",")
                                .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
        ...
        GroupBox("Today's Planner Chores") {
            VStack(alignment: .leading, spacing: 10) {
                HStack {
                    ...
                    Button("Load") { state.loadTodaysChoresIfPossible() }
                }

                if state.chores.isEmpty {
                    Text("No matching planner markdown for today. Use custom names and labels.")
                        .foregroundStyle(.secondary)
                } else {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Picker("Frequency", selection: $draft.frequency) {
                ForEach(HabitFrequency.allCases) { freq in Text(freq.rawValue.capitalized).tag(freq) }
            }
            Picker("Type", selection: $draft.metric) {
                ForEach(HabitMetric.allCases) { metric in Text(metric.displayName).tag(metric) }
            }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            LazyVGrid(columns: columns, spacing: 6) {
                ForEach(currentMonthDates(), id: \.self) { day in dayCell(day) }
            }
            Spacer()
        }
    ...
    private func dayCell(_ day: Date) -> some View {
        ...
        return VStack(spacing: 2) {
            ...
            RoundedRectangle(cornerRadius: 4)
                .fill(AppTheme.secondary.opacity(0.2 + intensity * 0.75))
                .frame(height: 14)
        }
        .padding(2)
        .background(Calendar.current.isDate(day, inSameDayAs: date) ? AppTheme.accent.opacity(0.2) : .clear)
...
final class Persistence {
    ...
    func appendSessionLog(_ log: SessionLog) {
        ...
        if !FileManager.default.fileExists(atPath: sessionCSVURL.path) {
            try? "id,mode,name,labels,start,end,configuredMinutes\n".write(to: sessionCSVURL, atomically: true, encoding: .utf8)
        }
        if let handle = try? FileHandle(forWritingTo: sessionCSVURL) {
            handle.seekToEndOfFile()
            handle.write(Data(csvLine.utf8))
            handle.closeFile()
        }
    }
